...
- OpenSSL untuk verifikasi eksternal )
...
- Install library yang dipakai program
...
- Jalankan program untuk generate semua komponen PKI                                                                                                                                                     
    ```python pubkey_infrastruktur.py --cn localhost --san localhost --outdir artifacts```
...
Verifikasi dilakukan dua lapis internal dan external 
- Verifikasi internal dari program
    ...
    - Masa berlaku valid: tanggal sekarang masih dalam rentang validitas sertifikat
      Yang tampil di output:
        - Signature valid (CA -> Server Cert) : True
        - Issuer cocok dengan Subject CA : True
        -  Masa berlaku valid saat ini : True

- Verifikasi eksternal pakai OpenSSL
Ini simulasi cara tool nyata memvalidasi chain:
```
...
Langkah 3 — Analisis PKI (Menghubungkan ke HTTPS/TLS)
Di langkah ini menjelaskan hubungan hasil praktikum dengan dunia nyata
...
Pada percobaan ini, Root CA dibuat terlebih dahulu sebagai “penerbit tepercaya”. Server tidak langsung membuat sertifikatnya sendiri, melainkan membuat CSR (Certificate Signing Request) yang berisi identitas dan public key. CSR kemudian ditandatangani oleh CA sehingga server certificate memiliki Issuer = CA. Ini menggambarkan konsep PKI yang sebenarnya: public key server dipercaya karena ada tanda tangan dari CA.
...
![Hasil pki](screenshots/pki.png)
![Hasil openssl](screenshots/openssl.png)
![Hasil Ovalidasi_chain](screenshots/validasi_chain.png)
)
